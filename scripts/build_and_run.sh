...
kill_all_server(){
    # use adb shell pidof droidgate get pid
    echo "尝试停止所有 DroidGate 相关进程"
    pidof_result=$(adb shell pidof droidgate)
    if [ -z "$pidof_result" ]; then
        echo "没有找到 DroidGate 相关进程"
    else
        echo "找到 DroidGate 相关进程 PID:$pidof_result 停止中"
...
# # 取MD5的前8位
SERVER_PATH=$LOCAL_DIR/build/droidgate-server
MD5=$(md5sum $SERVER_PATH | cut -d ' ' -f1 | cut -c 1-8)
NAP="$HOME/Desktop/nightmare-app"
ADB_KIT_ASSETS="$NAP/adb_kit/assets/droidgate-server"
UNCON_ASSETS="$NAP/uncon/assets/droidgate-server"
SULA_ASSETS="$NAP/neo_desktop/droidgate-server"
...
# adb shell app_process -Djava.net.preferIPv4Stack=true -Djava.library.path=/data/local/tmp/ -Djava.class.path=/data/local/tmp/droidgate-server$MD5 /system/bin --nice-name=droidgate com.nightmare.droidgate.DroidGate sula
