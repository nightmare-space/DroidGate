...
DroidGate 是一个轻量、插件化的 Android 系统能力网关。它通过统一的 HTTP 接口，将 Android Framework、Binder 服务、隐藏 API 和原生能力提供给外部客户端。
...
DroidGate 提供了封装好的开箱即用的 [Flutter Plugin](https://github.com/nightmare-space/droidgate_flutter)，或者你可以根据 [API.md](docs/API.md) 实现任意语言编写的客户端
...
- Flutter Plugin 支持: 只需要引入 Flutter 依赖，DroidGate 会随插件的注册而启动，在 Flutter 侧只需要调用 Dart API 即可
...
由于 HTTP 并不安全，所以 DroidGate 内置了一个简单的接口鉴权，来防止端口扫描恶意调用
...
这种情况下，DroidGate 拥有真实的 Activity Context，对于获取应用列表，同普通安卓本身访问 API一样，需要申请权限
...
DroidGateClient 是多实例，所有的 API 被封装到 DroidGateClient 下
...
开发调试启动脚本为 [build_and_run.sh](scripts/build_and_run.sh)
...
我们可以在连接到 PC 的设备上启动服务，再通过 `adb forward` 访问通信端口（`build_and_run.sh` 已包含端口转发）
...
提供 `droidgate_client` 来快速的让 Flutter App 拥有这个能力，无需手动启动服务，`DroidGate` 随 Flutter Plugin 注册而启动，直接创建 `DroidGateClient` 则会使用 Flutter Plugin 中启动的端口
...
示例代码中包含了所有的 API 使用方法，示例代码是最好能理解 DroidGate 的方式
...
完整代码见 [Flutter Example](https://github.com/nightmare-space/droidgate_flutter/tree/main/example)

## 开源仓库介绍

- droidgate-core：网关运行时和插件基础，不包含具体能力插件
- droidgate-hidden-api：供核心和插件编译使用的 Android 隐藏 API 桩
- droidgate-plugins：内置应用、任务、显示、输入、文件和用户管理等能力插件
- droidgate-bundle：注册全部内置插件并提供 `DroidGate` 统一入口的完整发行包
...
## 展望
我一直觉得腾讯的 PerfDog 收费太贵，使用 DroidGate，我觉得应该是能够编写一个 PerfDog? 来获得安卓端的帧率信息
还有 Scene，LibChecker，可以用 DroidGate 来将其支持到 PC 端，甚至 Web 端


## 谁在用？
- [Speed Share](https://github.com/nightmare-space/speed_share): `DroidGate` 以 Activity Mode的方式集成到速享，以让速享可以在 Flutter 侧获取应用列表，来选择一个应用发送到其它设备
- [ADB KIT](https://github.com/nightmare-space/adb_kit): `DroidGate` 在 ADB KIT 中同时以 Activity Mode 与 Dex Mode 存在，前者与速享类似，当我们需要安装本机已经安装的Apk到连接的设备后，就使用的这种模式；而当一个设备连接成功后，ADB KIT 便会通过 app_process 启动 Dex Mode 的 DroidGate，以实现 adb 命令行无法直接实现的功能，例如文件查看、预览
- Uncon(闭源): 同时以 Activity Mode 与 Dex Mode 存在，行为与 ADB KIT 类似，启动 Dex Mode 的 DroidGate 后，无界会用来加载目标设备运行的 Task
